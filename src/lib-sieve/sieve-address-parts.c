...
static bool
addrp_validator_load(const struct sieve_extension *ext,
...
static bool
sieve_address_part_exists(struct sieve_validator *valdtr,
...
static const struct sieve_address_part *
sieve_address_part_create_instance(struct sieve_validator *valdtr,
...
void sieve_address_parts_link_tags(struct sieve_validator *valdtr,
				   ...
				   int id_code)
{
	...
	sieve_validator_register_tag(valdtr, cmd_reg, adrp_ext,
				     &address_part_tag, id_code);
}
...
static bool
tag_address_part_is_instance_of(struct sieve_validator *valdtr,
				struct sieve_command *cmd,
				const struct sieve_extension *ext,
				const char *identifier, void **data);
static bool
tag_address_part_validate(struct sieve_validator *valdtr,
			  struct sieve_ast_argument **arg,
			  struct sieve_command *cmd);
static bool
tag_address_part_generate(const struct sieve_codegen_env *cgenv,
			  struct sieve_ast_argument *arg,
			  struct sieve_command *cmd);
...
static bool
tag_address_part_validate(struct sieve_validator *valdtr ATTR_UNUSED,
			  struct sieve_ast_argument **arg,
			  struct sieve_command *cmd ATTR_UNUSED)
{
	/* NOTE: Currenly trivial, but might need to allow for further
		 validation for future extensions.
	 */
...
static int
sieve_address_part_stringlist_next_item(struct sieve_stringlist *_strlist,
					string_t **str_r);
static void
sieve_address_part_stringlist_reset(struct sieve_stringlist *_strlist);
static int
sieve_address_part_stringlist_get_length(struct sieve_stringlist *_strlist);
static void
sieve_address_part_stringlist_set_trace(struct sieve_stringlist *_strlist,
					bool trace);
...
sieve_address_part_stringlist_create(const struct sieve_runtime_env *renv,
				     const struct sieve_address_part *addrp,
				     struct sieve_address_list *addresses)
{
...
int sieve_addrmatch_opr_optional_dump(const struct sieve_dumptime_env *denv,
...
int sieve_addrmatch_opr_optional_read(const struct sieve_runtime_env *renv,
				      sieve_size_t *address,
				      ...
				      struct sieve_comparator *cmp)
{
	...
	while (status == SIEVE_EXEC_OK) {
		...
		case SIEVE_AM_OPT_ADDRESS_PART:
			if (addrp == NULL) {
				sieve_runtime_trace_error(
					renv, "unexpected address-part operand");
				if (exec_status != NULL)
					*exec_status = SIEVE_EXEC_BIN_CORRUPT;
				return -1;
			}
			status = sieve_opr_address_part_read(
				renv, address, addrp);
			break;
		default:
			if (final) {
...
static const char *
addrp_all_extract_from(const struct sieve_address_part *addrp ATTR_UNUSED,
...
static const char *
addrp_domain_extract_from(const struct sieve_address_part *addrp ATTR_UNUSED,
			  const struct smtp_address *address)
{
...
static const char *
addrp_localpart_extract_from(const struct sieve_address_part *addrp ATTR_UNUSED,
			     const struct smtp_address *address)
{
...
const struct sieve_address_part_def domain_address_part = {
	SIEVE_OBJECT("domain", &address_part_operand,
