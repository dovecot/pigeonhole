...
void sieve_ext_mailbox_register_create_tag(
	...
	const struct sieve_extension *mailbox_ext, const char *command)
{
	if (sieve_validator_extension_loaded(valdtr, mailbox_ext)) {
		sieve_validator_register_external_tag(
			valdtr, command, mailbox_ext, &mailbox_create_tag,
...
static bool
ext_mailbox_validator_load(const struct sieve_extension *ext,
			   struct sieve_validator *valdtr);
...
ext_mailbox_validator_load(const struct sieve_extension *ext,
			   struct sieve_validator *valdtr)
{
	/* Register :create tag with fileinto command and we don't care whether
	   this command is registered or even whether it will be registered at
	   all. The validator handles either situation gracefully.
	 */
	sieve_validator_register_external_tag(
		valdtr, "fileinto", ext, &mailbox_create_tag,
		SIEVE_OPT_SIDE_EFFECT);
