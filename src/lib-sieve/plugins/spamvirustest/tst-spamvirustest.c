...
static bool
tst_spamvirustest_validate(struct sieve_validator *valdtr,
			   struct sieve_command *tst);
static bool
tst_spamvirustest_generate(const struct sieve_codegen_env *cgenv,
			   struct sieve_command *ctx);
static bool
tst_spamvirustest_registered(struct sieve_validator *valdtr,
...
static bool
tst_spamtest_validate_percent_tag(struct sieve_validator *valdtr,
...
static bool
tst_spamvirustest_operation_dump(const struct sieve_dumptime_env *denv,
				 sieve_size_t *address);
static int
tst_spamvirustest_operation_execute(const struct sieve_runtime_env *renv,
				    sieve_size_t *address);
			     ...
			     struct sieve_command_registration *cmd_reg)
{
	...
	if (sieve_extension_is(ext, spamtestplus_extension) ||
		...
		sieve_validator_register_tag(
			valdtr, cmd_reg, ext, &spamtest_percent_tag,
			OPT_SPAMTEST_PERCENT);
	}
	return TRUE;
}
			   ...
			   struct sieve_command *tst)
{
	...
	if (!sieve_validate_positional_argument(valdtr, tst, arg, "value", 1,
						SAAT_STRING))
		return FALSE;
				 ...
				 sieve_size_t *address)
{
	...
	return sieve_opr_string_dump(denv, address, "value");
}
				    ...
				    sieve_size_t *address)
{
	...
	/* Read value part */
	ret = sieve_opr_stringlist_read(renv, address, "value", &key_list);
	if (ret <= 0)
		return ret;
	...
	if (sieve_operation_is(op, spamtest_operation)) {
		sieve_runtime_trace(renv, SIEVE_TRLVL_TESTS,
				    "spamtest test [percent=%s]",
				    (percent ? "true" : "false"));
	} else {
		sieve_runtime_trace(renv, SIEVE_TRLVL_TESTS, "virustest test");
	}

	/* Get score value */
	sieve_runtime_trace_descend(renv);
	ret = ext_spamvirustest_get_value(renv, this_ext, percent,
					  &score_value);
	if (ret <= 0)
		return ret;
